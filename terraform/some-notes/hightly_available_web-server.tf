## https://github.com/adv4000/terraform-lessons/blob/master/Lesson-11-ALB-LaunchTemplate/main.tf

provider "aws" {
  region = "eu-north-1"

  default_tags {                                                 // tags to added to each resource 
    tags = {
      Owner     = "Nat"
      CreatedBy = "Terraform"
...
data "aws_availability_zones" "working" {}                        // get our a-zones

data "aws_ami" "latest_amazon_linux" {                            // get latest ami                 
  most_recent = true
  owners      = ["amazon"]
...
resource "aws_default_vpc" "default" {}                           // vpc should be defined, even default
